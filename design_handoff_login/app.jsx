    ...
    <div style={{ position: 'fixed', inset: 0, display: 'flex', alignItems: 'center',
      ...
      <div style={{ transform: `scale(${scale})`, transformOrigin: 'center center' }}>
        <div style={{ ...vars, fontFamily: 'var(--font-body)' }}>
          <IOSDevice dark={authed ? t.dark : true}>
            {!authed ? (
              <LoginScreen key={(t.loginMethod || 'face') + String(t.loginAura)}
                defaultMethod={t.loginMethod || 'face'} aura={t.loginAura !== false}
                onSuccess={() => { setAuthed(true); setTab('home'); }} />
            ) : (
      ...
      <TweaksPanel>
        ...
        <TweakSection label="Connexion" />
        <TweakRadio label="Méthode par défaut" value={t.loginMethod || 'face'}
          options={[{ value: 'face', label: 'Face ID' }, { value: 'pwd', label: 'Identifiants' }]}
          onChange={(v) => setTweak('loginMethod', v)} />
        <TweakToggle label="Fond animé" value={t.loginAura !== false}
          onChange={(v) => setTweak('loginAura', v)} />
        {authed && <TweakButton label="Revoir l'écran de connexion" secondary
