...
export function useAdminActions() {
  ...
  const approveAbsenceMutation = useMutation({
    mutationFn: (id: string) => updateAbsenceStatus(id, 'JUSTIFIED', 'Approuve depuis mobile'),
    onSuccess: invalidate,
  });

  const rejectAbsenceMutation = useMutation({
    mutationFn: (id: string) => updateAbsenceStatus(id, 'UNJUSTIFIED', 'Rejete depuis mobile'),
    onSuccess: invalidate,
  });
